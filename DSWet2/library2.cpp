...
{
    void* r = (void*)(new Reserve(n,levels));
    return r;
}
...
{
    if(DS==NULL||magiID<=0||magiLevel<=0)
        return INVALID_INPUT;
    StatusType st = ((Reserve*)DS)->addMagizoologist(magiID,magiLevel);
...
StatusType RemoveMagizoologist(void *DS, int magiID)
{
    if(DS==NULL||magiID<=0)
        return INVALID_INPUT;
    StatusType st = ((Reserve*)DS)->removeMagizoologist(magiID);
    //if(st==ALLOCATION_ERROR)
  //      return ALLOCATION_ERROR;
    return st;
}
...
{
    if(DS==NULL||creature1<0||creature2<0)
        return INVALID_INPUT;
    StatusType st = ((Reserve*)DS)->removeBarrier(creature1,creature2);
    return st;
}
...
{
    if(DS==NULL||creatureID<0)
        return INVALID_INPUT;
    StatusType st = ((Reserve*)DS)->assignMagizoologistToCreature(creatureID);
    //if(st==ALLOCATION_ERROR)
   //     return ALLOCATION_ERROR;
    return st;
}
...
StatusType ReleaseMagizoologist(void *DS, int magiID)
{
    if(DS==NULL||magiID<=0)
        return INVALID_INPUT;
    StatusType st = ((Reserve*)DS)->releaseMagizoologist(magiID);
    return st;
...
StatusType GetCreatureOfMagi(void *DS, int magiID, int* creatureID)
{
    if(DS==NULL||magiID<=0)
        return INVALID_INPUT;
    StatusType st = ((Reserve*)DS)->getCreatureOfMagi(magiID,creatureID);
    return st;
}
...
{
    if(DS==NULL||creature1<0||creature2<0||!sameArea)
        return INVALID_INPUT;
    StatusType st = ((Reserve*)DS)->areCreatureInSameArea(creature1,creature2,sameArea);
    return st;
}
...
{
    if(DS==NULL||creatureID<0||!sizeOfArea)
        return INVALID_INPUT;
    StatusType st = ((Reserve*)DS)->getSizeOfArea(creatureID,sizeOfArea);
    return st;
}
